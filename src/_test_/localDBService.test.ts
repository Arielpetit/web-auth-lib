...
describe('LocalDBService', () => {
  ...
  test('should encrypt and decrypt a message correctly', async () => {
    ...
    const encryptedMessage = await localDBService.encryptData(message, encryptionKey);
    const decryptedMessage = await localDBService.decryptData(encryptedMessage, encryptionKey);
  ...
  test('should throw error if decryption fails', async () => {
    ...
    await expect(localDBService.decryptData(encryptedMessageArray.buffer, encryptionKey)).rejects.toThrow('Decryption error');
  });
});
